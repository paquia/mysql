...
-- tbl_menu 에서 메뉴코드랑 메뉴이름, 메뉴가격을 내림차순으로 조회
select
	menu_code, menu_name, menu_price
from tbl_menu
order by
...
select
	menu_code, menu_price, menu_code * menu_price as 연산결과
from tbl_menu
order by 
	menu_code * menu_price desc;
    
-- order by 절 사용하여 사용자 지정 목록을 사용하여 데이터 정렬

-- 맨 왼쪽 값이 2번째 인자 이후의 값과 일치하면 해당위치 값을 반환
...
-- tbl_meun 테이블에서 메뉴이름, 판매상태를 조회하는데 판매상태가 N인 친구들이 위로 오게
select

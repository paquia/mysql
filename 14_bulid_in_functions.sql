...
-- locate : instr 과 동일하고 순서만 반대alter

select
	elt(2, '사과', '딸기', '바나나'), field('딸기', '사과', '딸기', '바나나'), find_in_set('바나나', '사과, 딸기, 바나나'), instr('사과딸기바나나', '딸기'), locate('딸기', '사과딸기바나나');
    
-- format(숫자, 소수점 자리수)
-- 1000 단위마다 콤마(,) 표시를 해주며 소수점 아래 자릿수 까지 반올림 표현한다.

 select format(12341253212.312122142, 3)
 
 -- insert 
 
 select insert('내 이름은 OOO 입니다.', 7, 3, '박준혁');
 
 -- left, right
 -- left : 왼쪽에서 문자열의 길이만큼 반환
 -- right : 오른쪽에서 문자열의 길이만큼 반환
 select left('hello world!', 3), right('hello new world', 3);
 
 -- upper, lower
 select upper('Hello World!'), lower('Hellow World!');
...
-- addtime(날짜/시간, 시간)
-- subtime(날짜/시간, 시간)
select addtime('2024-03-04 17:42:00', '0:30:0');
...
select year(curtime()), month(curtime()), microsecond(curtime());

-- datediff(날짜 1, 날짜2)
-- timediff(날짜 1 or 시간 1, 날짜 2 또는 시간 2)
-- datediff : 날짜 1 - 날짜 2의 일수 반환
...
-- dayofyear : 해당 년도에서 며칠이 흘렀는지

select dayofweek(curdate()), monthname(curdate()), dayofyear(curdate());

-- last_day(날짜)
-- 해당 날짜의 달에서 마지막날의 날짜를 구한다.
